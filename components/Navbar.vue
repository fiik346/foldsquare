<template>
  <nav class="bg-white shadow sticky top-0">
    <div id="search" :class="search ? 'top-0' : '-top-full'">
      <div
        class="container z-10 relative mt-4 mx-auto max-w-lg p-4 lg:p-8 bg-white rounded"
      >
        <form class="flex">
          <input
            id="searchinput"
    ...
    <div class="container header">
      <button
        ...
        class="h-full -ml-3 p-3 focus:outline-none lg:hidden"
        @click="menuF()"
      >
        <svg
      ...
      <div class="mx-auto lg:ml-0">
        <nuxt-link to="/" class="text-2xl font-serif font-bold"
          >Portal Wakanda</nuxt-link
        >
      </div>
      <label
        role="button"
        for="searchinput"
        class="h-full -mr-3 p-3 focus:outline-none flex items-center"
    ...
    >
      <ul class="container mx-auto flex flex-wrap lg:flex-nowrap">
        <li v-for="(item, i) in nav" :key="i" class="w-6/12 lg:w-max">
          <nuxt-link
            :to="item.to"
            class="text-white text-sm uppercase font-bold block px-3 py-1 hover:bg-gray-900 focus:bg-gray-800 lg:bg-white lg:text-black lg:hover:bg-white lg:focus:bg-white lg:hover:text-gray-800"
